...
// Inicia en la canción 1 (la que ya tienes descargada)
let currentIndex = 0;
...
// Dibuja la lista y asegura que la seleccionada esté SIEMPRE en amarillo
function renderList() {
    ...
    `;
        ...
        // Al hacer clic, actualiza la interfaz AL INSTANTE
        btn.onclick = () => {
            selectAndPlay(idx);
        };
...
function selectAndPlay(index) {
    ...
    // 1. PINTAR EN AMARILLO INMEDIATAMENTE (sin esperar al archivo de audio)
    ...
    // 3. Reproducir con protección contra errores
    ...
    if (playPromise !== undefined) {
        playPromise
            .then(() => {
                isAudioPlaying = true;
                if (btnPlay) btnPlay.textContent = "❚❚";
                renderList(); // Muestra el ecualizador activo
            })
            .catch((error) => {
                // Si el archivo no existe aún (ej: 08.mp3), no rompemos nada
                console.warn(`El archivo ${song.src} aún no está disponible o fue bloqueado:`, error.message);
                isAudioPlaying = false;
                if (btnPlay) btnPlay.textContent = "▶";
                renderList();
            });
        ...
        if (audio.paused) {
            audio.play()
                ...
                .catch((err) => {
                    ...
                    alert(`No se encontró el archivo "${songs[currentIndex].src}". Asegúrate de haberlo subido a la carpeta songs/`);
                });
        } else {
            audio.pause();
            isAudioPlaying = false;
